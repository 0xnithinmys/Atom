  ...
  const sendMessage = useCallback(async (userText: string) => {
    ...
    try {
      ...
      if (!accumulated) {
        setMessages((prev) => [
          ...prev.slice(0, -1),
          { id: assistantId, role: "assistant", content: "I received your message but could not generate a response. Please try again." },
        ]);
      }
    } catch (err: unknown) {
    ...
    >
      <div className="shrink-0 bg-gradient-to-r from-indigo-600/90 via-violet-600/90 to-purple-700/90 backdrop-blur-sm px-4 py-3 flex items-center justify-between border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-white leading-none">{ASSISTANT_NAME}</h3>
            <p className="text-xs text-indigo-200 mt-0.5 leading-none">
              {isLoading ? "Thinking..." : `${ASSISTANT_MODEL_LABEL} · ${ASSISTANT_PROVIDER}`}
            </p>
          </div>
        </div>
        ...
        <>
          <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-4 bg-gradient-to-b from-slate-900/80 to-slate-950/80">
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center flex-1 text-center space-y-5 py-6">
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-200">You are chatting with {ASSISTANT_NAME}</p>
                  <p className="text-xs text-slate-400 mt-1 leading-relaxed max-w-[240px] mx-auto">
                    I can help with goal writing, achievement logs, feedback, and navigation in {APP_NAME}.
                  </p>
                </div>
                    ...
                    message.role === "user" ? (
                      ...
                      <ReactMarkdown
                        ...
                        components={{
                          ...
                          ol: ({ children }) => <ol className="list-none space-y-1 my-2 pl-0">{children}</ol>,
                          li: ({ children }) => (
          ...
          <div className="shrink-0 border-t border-indigo-500/20 bg-slate-900/80 p-3">
            ...
            <p className="text-center text-slate-600 text-[10px] mt-2">
              {ASSISTANT_MODEL_LABEL} · {ASSISTANT_PROVIDER} · Responses may contain errors
            </p>
          </div>
        </>
      )}
